...
class DQ3Analyzer:
	...
	def generate_progress_report(self) -> Dict[str, Any]:
		...
		report = {
			...
			'functions': {
				'total': len(self.disassembler.functions),
				'by_type': {},
				'by_bank': {}
			},
			'memory_map': {
	...
	def _update_progress_metrics(self):
		...
		self.progress.functions_analyzed = len([f for f in self.disassembler.functions.values()
											  if f.function_type != "unknown"])
	...
	def export_analysis(self, output_dir: str, level: AnalysisLevel = AnalysisLevel.COMPREHENSIVE):
		...
		# Memory map
		memory_map_data = {
			'regions': [asdict(region) for region in self.memory_map]
		}
		...
		systems_data = {
			name: asdict(system) for name, system in self.game_systems.items()
		}
	...
	def _generate_analysis_report(self, output_path: Path, analyses: Dict[str, Any]):
		...
		report_lines.extend([
			"🏦 BANKING SYSTEM",
			"-" * 40,
			f"Total Banks: {analyses['banking']['total_banks']}",
			""
		])
...
def main():
	...
	parser.add_argument("--level", choices=["basic", "intermediate", "comprehensive", "exhaustive"],
