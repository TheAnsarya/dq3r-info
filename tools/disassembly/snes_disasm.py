...
class SNES65816Disassembler:
	...
	def analyze_function(self, start_addr: int, bank: int = 0) -> Optional[Function]:
		...
		function = Function(
			...
			name=f"sub_{bank:02X}_{start_addr:04X}",
			bank=bank
		)
		...
		for _ in range(max_size):
			...
			if instruction.mnemonic in ['RTS', 'RTL', 'RTI']:
			...
			elif instruction.mnemonic in ['JMP', 'JML'] and \
				instruction.addressing_mode in [AddressingMode.ABSOLUTE, AddressingMode.ABSOLUTE_LONG]:
				...
				target = instruction.operand_value
				if target:
					branch_target = (current_addr + instruction.size +
								   struct.unpack('<b', instruction.operand_bytes)[0]) & 0xFFFF
	...
	def _classify_function(self, function: Function) -> str:
		...
		if len(function.instructions) > 50 and any(inst.mnemonic in ['BRA', 'JMP'] for inst in function.instructions[-5:]):
			return "main_loop"
