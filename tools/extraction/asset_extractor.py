...
class DQ3AssetExtractor:
			...
			if location['start'] < self.rom_size:
				...
				if data and not self._is_empty_data(data):
					asset = ExtractedAsset(
						...
						data=data,
						metadata={
							'tile_count': actual_size // 32 if location['format'] == AssetFormat.SNES_4BPP else actual_size // 16,
	...
	def _generate_extraction_report(self, output_path: Path, results: Dict[str, Any]):
		...
		for category in ['graphics', 'audio', 'text', 'data']:
			...
			report_lines.extend([
				f"## {category.title()} Assets ({count} items)",
				""
			])
		...
		for i, asset in enumerate(self.extracted_assets, 1):
			category = 'graphics' if 'sprite' in asset.name or 'tile' in asset.name else \
					  'audio' if 'music' in asset.name or 'sound' in asset.name else \
					  'text' if 'dialog' in asset.name or 'menu' in asset.name else 'data'
...
def main():
	...
	parser.add_argument("--category", choices=['graphics', 'audio', 'text', 'data', 'all'],
						default='all', help="Asset category to extract")
