...
class DQ3ProgressTracker:
	...
	def generate_progress_table(self) -> str:
		...
		extracted_assets = sum(sum(1 for asset in assets if asset.extracted) for assets in self.asset_inventory.values())
...
<body>
	...
	def export_progress_reports(self, output_dir: str):
		...
		export_data = {
			...
			'asset_inventory': {k.value: [asset_to_dict(asset) for asset in v] for k, v in self.asset_inventory.items()},
